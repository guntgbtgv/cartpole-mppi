      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (elapsedSim < 0 || elapsedCPU.count() < 0 || syncCPU.time_since_epoch().count() == 0 ||
            ...
            mppi_controller.read(d->time, d->qpos, d->qvel);
            mppi_controller.control_mujoco();
            mppi_controller.write(d->ctrl);
...
int main(int argc, char** argv) {
  ...
  const char* filename = "../suite/cartpole.xml";
  if (argc >  1) {
    filename = argv[1];
  }
